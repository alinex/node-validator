...
class NumberSchema extends AnySchema {
  ...
  _unitValidator(data: Data): Promise<void> {
    const check = this._check
    if (check.unit) {
      try {
 ...
 ${set.integerType}-bit integer. `
  ...
  _minmaxValidator(data: Data): Promise<void> {
    const check = this._check
    try {
 ...
 ${set.round.precision} digits precision.\n`
  ...
  _roundValidator(data: Data): Promise<void> {
    const check = this._check
    try {
...
${this._isReference('multiple') ? set.multiple.description : set.multiple}.\n`
  ...
  _multipleValidator(data: Data): Promise<void> {
    const check = this._check
    try {
